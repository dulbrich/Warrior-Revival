...
The `/gallery` page is bucket-as-source-of-truth: no DB table, photos live in `public-images/gallery/` and are listed at render time via `supabase.storage.from('public-images').list('gallery', ...)` (see `src/lib/gallery/queries.ts`). Storage policies in `supabase/migrations/0004_gallery_storage.sql` allow public read on `gallery/*` and admin-only writes. The admin UI at `/admin/gallery` includes `StorageUsageBar` (`src/components/StorageUsageBar.tsx` → `src/lib/storage/usage.ts`) which sums sizes across every bucket via the service-role client and renders a project-wide bar (free-tier cap = 1 GB). Gallery uploads happen client-side via XHR so admins see per-file progress bars; see `src/lib/gallery/clientUpload.ts` and the `GalleryUploadForm` client component. Authz is RLS — the user's session token carries `is_admin()`.

The "What Our Members Say" carousel on `/veterans` is fed by the Supabase `testimonials` table (see `supabase/migrations/0005_testimonials.sql`). Admin CRUD at `/admin/testimonials` with manual up/down ordering via `sort_order`. `fetchTestimonials()` in `src/lib/testimonials/queries.ts` returns a plain `string[]` so the existing carousel rendering in `VeteransPageClient.tsx` stays unchanged.
